...
echo "Zodiac key is" . searchZodiac($zodiacArray,$dateofBirth);
/*
    function getZodiac() {

        $zDate = date("m-d", strtotime(dateofBirth));
        foreach ($zodiacArray as $key => $val) {
            if ($zDate >= $val['start'] && $zDate <= $val['end']) {
                return $val['key'];
    }}}
 */